...
task.delay(4, function()
  ...
  function Rayfield:RemoveElement(element)
    ...
    pcall(function()
        -- If it's a button
        if element.ButtonFrame and element.ButtonFrame.Parent then
            element.ButtonFrame:Destroy()
        end
        ...
        -- If it's an input
        if element.InputFrame and element.InputFrame.Parent then
            element.InputFrame:Destroy()
        end
        
        -- Fallback general destruction
...
function Rayfield:ClearSearchResults(resultsTable)
    if not resultsTable then return end
    
    -- Remove each element
    for i = #resultsTable, 1, -1 do
        local element = resultsTable[i]
        self:RemoveElement(element)
        table.remove(resultsTable, i)
    end
end
